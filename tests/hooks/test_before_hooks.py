...
def test_api_key_auth_and_ip_override(app, client):
    ...
    response = client.get(
        '/test_api_key',
        environ_base={'HTTP_USER_AGENT': '', 'REMOTE_ADDR': '127.0.0.1'},
...
def test_user_bad_api_key(app, client, authorization_header):
    ...
    response = client.get(
        '/users/1', headers={'Authorization': authorization_header}
    )
    check_json_response(response, 'Invalid authorization.')


@pytest.mark.parametrize(
    'endpoint', ['/users/change_password', '/not/a/real/route']
)
...
def test_rate_limit_fail(app, client, monkeypatch):
    ...
    check_json_response(
        response, 'Client rate limit exceeded. 7 seconds until lock expires.'
    )
...
def test_rate_limit_user_fail(app, client, monkeypatch):
    ...
    check_json_response(
        response, 'User rate limit exceeded. 7 seconds until lock expires.'
    )
