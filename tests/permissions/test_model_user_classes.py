...
@pytest.mark.parametrize(
    'class_, name', [(UserClass, 'UsEr'), (SecondaryClass, 'Fls')]
)
def test_create_dupe_user_classes(app, client, class_, name):
    with pytest.raises(APIException):
        class_.new(name=name, permissions=None)


@pytest.mark.parametrize(
    'class_, name', [('user_classes', 'USeR'), ('secondary_classes', 'fLS')]
)
...
@pytest.mark.parametrize('class_', [UserClass, SecondaryClass])
...
def test_user_secondary_classes_models(app, client):
    cache.set(
        SecondaryClass.__cache_key_of_user__.format(id=1), [2], timeout=60
    )
