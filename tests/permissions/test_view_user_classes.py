...
def test_view_user_class(app, authed_client):
    ...
    assert set(response['permissions']) == {
        'permissions_modify',
        'users_edit_settings',
    }
...
def test_view_user_class_secondary(app, authed_client):
    response = authed_client.get(
        '/user_classes/2', query_string={'secondary': True}
    ).get_json()
    assert 'response' in response
    response = response['response']
...
def test_create_user_class_duplicate(app, authed_client):
    response = authed_client.post(
        '/user_classes',
        data=json.dumps({'name': 'Power User', 'permissions': []}),
    ).get_json()
    assert (
        response['response']
        == 'Another UserClass already has the name Power User.'
    )
...
def test_create_secondary_class_duplicate(app, authed_client):
    response = authed_client.post(
        '/user_classes',
        data=json.dumps(
            {'name': 'Beans Team', 'permissions': [], 'secondary': True}
        ),
    ).get_json()
    assert (
        response['response']
        == 'Another SecondaryClass already has the name Beans Team.'
    )
...
def test_delete_secondary_with_uc_name(app, authed_client):
    response = authed_client.delete(
        '/user_classes/5', query_string={'secondary': True}
    ).get_json()
...
def test_delete_secondary_class_with_user(app, authed_client):
    response = authed_client.delete(
        '/user_classes/1', query_string={'secondary': True}
    ).get_json()
    assert (
        response['response']
...
def test_modify_secondary_user_class(app, authed_client):
    authed_client.put(
        '/user_classes/2',
        data=json.dumps(
            {'permissions': {'users_edit_settings': False}, 'secondary': True}
        ),
    )
...
def test_modify_user_class_failure(app, authed_client, permissions, error):
    response = authed_client.put(
        '/user_classes/1', data=json.dumps({'permissions': permissions})
    ).get_json()
...
def test_modify_user_class_nonexistent(app, authed_client):
    response = authed_client.put(
        '/user_classes/7',
        data=json.dumps({'permissions': {'invites_send': True}}),
    ).get_json()
