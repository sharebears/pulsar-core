...
def test_moderate_user(app, authed_client):
    ...
    check_json_response(
        response,
        {'id': 2, 'email': 'new@ema.il', 'uploaded': 999, 'downloaded': 998},
    )
    user = User.from_pk(2)
...
def test_moderate_user_incomplete(app, authed_client):
    add_permissions(app, 'users_moderate')
    response = authed_client.put(
        '/users/2', data=json.dumps({'password': 'abcdefGHIfJK12#'})
    )
    check_json_response(
        response, {'id': 2, 'email': 'user_two@puls.ar', 'downloaded': 0}
    )
    user = User.from_pk(2)
...
def test_moderate_user_not_found(app, authed_client):
    add_permissions(app, 'users_moderate')
    response = authed_client.put(
        '/users/10', data=json.dumps({'email': 'new@ema.il'})
    )
...
def test_change_permissions_failure(app, authed_client, permissions, expected):
    ...
    response = authed_client.put(
        '/users/1', data=json.dumps({'permissions': permissions})
    )
...
@pytest.mark.parametrize('endpoint, method', [('/users/1', 'PUT')])
