...
def test_get_user_self_and_caches(app, authed_client):
    ...
    check_json_response(
        response,
        {'id': 1, 'username': 'user_one', 'secondary_classes': ['FLS']},
    )
...
def test_get_user(app, authed_client):
    ...
    check_json_response(
        response, {'id': 2, 'username': 'user_two', 'user_class': 'User'}
    )
    assert response.status_code == 200
...
def test_get_user_detailed(app, authed_client):
    ...
    assert any(
        ak['hash'] == 'abcdefghij' for ak in data['response']['api_keys']
    )
...
def test_users_edit_settings(
    app, authed_client, existing_password, new_password, message
):
...
def test_users_edit_settings_pw_fail(app, authed_client):
    ...
    response = authed_client.put(
        '/users/settings',
        data=json.dumps(
            {'existing_password': '12345', 'new_password': 'aB1%ckeofa12342'}
        ),
    )
    check_json_response(
...
def test_users_edit_settings_others(app, authed_client):
    add_permissions(
        app, 'users_edit_settings', 'users_change_password', 'users_moderate'
    )
    response = authed_client.put(
        '/users/settings', query_string={'user_id': 2}, data=json.dumps({})
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            'user_one',
            400,
            'Invalid data: another user already has the username '
...
def test_registration_no_code(app, client):
    ...
    check_json_response(
        response, 'An invite code is required for registration.'
    )


@pytest.mark.parametrize(
    'endpoint, method', [('/users/1', 'GET'), ('/users/settings', 'PUT')]
)
