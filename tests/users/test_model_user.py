...
def test_user_creation(app, client):
    ...
    user = User.new(
        username='bright', password='13579', email='bright@puls.ar'
    )
...
def test_user_creation_dupe_username(app, client):
    ...
    with pytest.raises(APIException) as e:
        User.new(username='user_OnE', password='13579', email='bright@puls.ar')
...
def test_locked_acc_perms_blocked(app, client):
    ...
    response = client.get(
        '/users/1', headers={'Authorization': f'Token bcdefghijk{CODE_3}'}
    )
